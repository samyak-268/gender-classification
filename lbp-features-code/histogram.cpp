...
template <typename _Tp>
void lbp::uniformPatternHistogram_(const Mat& src, Mat& hist, int numPatterns) {
	// Initialize histogram and mark bins corresponding to  non-uniform patterns.
...
void lbp::uniformPatternSpatialHistogram(const Mat& src, Mat& hist, int numPatterns, const Size& window, int overlap) {
    int width = src.cols;
	int height = src.rows;
	vector<Mat> histograms;
	for(int x=0; x <= (width - window.width); x+=(window.width-overlap)) {
...
void lbp::uniformPatternHistogram(const Mat& src, Mat& hist, int numPatterns) {
	switch(src.type()) {
		case CV_8SC1:
            uniformPatternHistogram_<char>(src, hist, numPatterns);
            break;
		case CV_8UC1:
            uniformPatternHistogram_<unsigned char>(src, hist, numPatterns);
            break;
		case CV_16SC1: 
            uniformPatternHistogram_<short>(src, hist, numPatterns);
            break;
		case CV_16UC1: 
            uniformPatternHistogram_<unsigned short>(src, hist, numPatterns); 
            break;
		case CV_32SC1: 
            uniformPatternHistogram_<int>(src, hist, numPatterns);
            break;
	}
}
...
void lbp::uniformPatternSpatialHistogram(const Mat& src, Mat& dst, int numPatterns, int gridx, int gridy, int overlap) {
    int width = static_cast<int>(floor(src.cols/gridx));
	int height = static_cast<int>(floor(src.rows / gridy));

	uniformPatternSpatialHistogram(src, dst, numPatterns, Size_<int>(width, height), overlap);
}
...
Mat lbp::uniformPatternHistogram(const Mat& src, int numPatterns) {
	Mat hist;
	uniformPatternHistogram(src, hist, numPatterns);
	return hist;
}
...
Mat lbp::uniformPatternSpatialHistogram(const Mat& src, int numPatterns, int gridx, int gridy, int overlap) {
	Mat hist;
	uniformPatternSpatialHistogram(src, hist, numPatterns, gridx, gridy);
	return hist;
}

vector<int> lbp::convertToBinary(int x) {
...
int lbp::countTransitions(vector<int> x) {
    int result = 0;
    for(int i = 0; i < 8; ++i)
        result += (x[i] != x[(i+1) % 8]);
    return result;
